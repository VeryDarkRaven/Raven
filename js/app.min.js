...
function screenControlArrows(blocksPageObj) {
    ...
    let currectPosition = rewriteCurrectBlock();
    hideInactiveButton();
    ...
    function checkDirectionButton(e) {
        ...
        for (const key in buttonDirectionAndClick) {
            if (button.classList.contains(key)) {
                clickButton(buttonDirectionAndClick[key]);
                break;
            }
        }
    }
    function clickButton(changePosition) {
    ...
    function rewriteCurrectBlock() {
        const elem = document.elementsFromPoint(0, 0).filter((item) => item.closest(".wrapper"))[0];
