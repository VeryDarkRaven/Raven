    ...
    function checkDirectionButton(e) {
        ...
        for (const key in buttonDirectionAndClick) {
            if (button.classList.contains(key)) {
                clickButton(buttonDirectionAndClick[key], key);
                break;
            }
        }
    }
    function clickButton(changePosition, direction) {
...
function sectionsHeight() {
    ...
    const sections = wrapper.querySelectorAll(".wrapper__child");
    sections.forEach((item) => {
        item.style.height = `${document.documentElement.clientHeight}px`;
    });
    const heihgtBlock = document.querySelector("#height");
    // heihgtBlock.textContent = `${document.documentElement.clientHeight}`;
    const innerHeight = window.innerHeight;
    const contentHeight = document.documentElement.clientHeight;
    heihgtBlock.textContent = `${innerHeight + " & " + contentHeight}`;
}
